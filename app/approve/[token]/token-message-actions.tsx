...
import {
  editAndApproveMessageByToken,
  rejectMessageByToken,
} from "@/lib/actions";

export function TokenMessageActions({
  token,
  messageId,
  body,
}: {
  ...
  if (mode === "edit") {
    return (
      <form action={editAndApproveMessageByToken} className="w-full space-y-2">
  ...
  if (mode === "reject") {
    return (
      <form action={rejectMessageByToken} className="w-full space-y-2">
        <input type="hidden" name="token" value={token} />
        <input type="hidden" name="messageId" value={messageId} />
        <textarea
          name="note"
