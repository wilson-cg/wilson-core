...
import { editAndApprovePostByToken, rejectPostByToken } from "@/lib/actions";

export function TokenPostActions({
  token,
  postId,
  body,
}: {
  ...
  if (mode === "edit") {
    return (
      <form action={editAndApprovePostByToken} className="w-full space-y-2">
  ...
  if (mode === "reject") {
    return (
      <form action={rejectPostByToken} className="w-full space-y-2">
        <input type="hidden" name="token" value={token} />
        <input type="hidden" name="postId" value={postId} />
        <textarea
          name="note"
          ...
          required
          minLength={3}
          placeholder="Tell the team what to change about this post"
