...
import { editAndApprovePost, rejectPost } from "@/lib/actions";
...
export function PostApprovalActions({
  ...
  if (mode === "edit") {
    return (
      <form action={editAndApprovePost} className="w-full space-y-2">
  ...
  if (mode === "reject") {
    return (
      <form action={rejectPost} className="w-full space-y-2">
        <input type="hidden" name="postId" value={postId} />
        <textarea
          name="note"
          value={note}
